    ...
    def resample(self):
        ...
        if not self.dataset().data.empty:
            ...
            if self.unit != dataset.display_unit:
                if self.agg_method not in ['ACCUMULATION (CFS to KAF)',
                                           'ACCUMULATION (CMS to MCM)']:
...
class ModelConfiguration:
    ...
    def __simple_target_string__(self):
        ...
        exclude_str = '(excluding ' + ','.join(
            list(map(str, self.training_exclude_dates))) + ')' if exclude else ''
