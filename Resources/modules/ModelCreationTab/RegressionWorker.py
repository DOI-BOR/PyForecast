...
if __name__ == '__main__':
    ...
    # Load data
    with open('../GIBR_2.fcst', 'rb') as readfile:
    ...
    predictorWindows = [
        "R/1985-03-01/P1M/F1Y",
        "R/1985-04-01/P1D/F1Y",
        "R/1985-04-01/P1D/F1Y",
        "R/1985-04-01/P1D/F1Y",
        "R/1985-04-01/P1D/F1Y",
        "R/1985-04-01/P1D/F1Y",
    ...
    # Initialize an entry in the modelRunsTable
    modelRunsTable.loc[0] = [
                "1985/2019/2011",           # Train models using data from 1985 - 2019, excluding the year 2011
                "April 01",                 # Forecast Issue date
                106187,                     # Dataset ID for GIBR Inflow data
                "R/1985-04-01/P121D/F1Y",   # We're forecasting the inflow accumulation for the period april 1st - july 31st (121 days).
                'accumulation_cfs_kaf',     # We'll accumulate the inflows and convert to KAF
                predictorPool,              # The predictor search space
                [False]*len(predictorPool),
                predictorWindows,
                predictorMethods,
                ['Regr_MultipleLinearRegressor', 'Regr_PCARegressor'],          # Regression algorithms to iterate through while generating models
                'KFOLD_5',                                                      # Cross validation method for model scoring and feature selection
                ['FeatSel_SequentialForwardFloating',],                         # Feature Selection algorithm
                ['MSE', 'ADJ_R2'],                                              # Models with high R2 and low MSE are scored the best
                ['PreProc_YAware']                                              # Use the Y Aware Pre-processor before creating models.
            ]

    # Fake a parent object that mimics the NextFlow Application structure
    class fake_parent(object):

        def __init__(self, datasetTable, dataTable, modelRunsTable):
            ...
            self.modelRunsTable = modelRunsTable
            self.datasetTable = datasetTable

            #

        def generateModels(self, i=0):
            self.rg = RegressionWorker(self, modelRunTableEntry=self.modelRunsTable.iloc[i])
            self.rg.setData()
            self.rg.run()

    # Initialize the parent object
    parentObj = fake_parent(datasetTable, dataTable, modelRunsTable)

    # Generate models
    parentObj.generateModels()

    # We now have a list of models, sorted by score, in the list parentObj.rg.resultsList
    # Print the best performing model to the screen
    bestModel = parentObj.rg.resultsList[0]
    print("\nA total of {0} models were assessed".format(len(parentObj.rg.resultsList)))
