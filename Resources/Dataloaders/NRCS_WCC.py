...
class Dataloader(object):
    ...
    def load(self, dataset, date1, date2):
        ...
        df = pd.DataFrame(
            index=pd.date_range(date1, date2))  # Create a dataframe to store values
        ...
        if soilFlag:
            ...
            df2[dataset.guid] = [pd.to_numeric(value, errors='coerce') for value in
                                 data[0]['values']]
            df2 = df2[~df2.index.duplicated(
                keep='first')]  # Remove duplicates from the dataset
        ...
        # Get all the data for SWE and PRECIP staitons and store in a dataframe
            ...
            if triplet[0].split(':')[2] == 'SNOW':
                ...
                if actualStartDate != None or actualEndDate != None:
                    df2 = pd.DataFrame(
                        index=pd.date_range(actualStartDate, actualEndDate))
                else:
                    df2 = pd.DataFrame()
            values = [pd.to_numeric(value, errors='coerce') for value in
                      data[0]['values']]
            df2[dataset.guid] = values
            df2 = df2[
                ~df2.index.duplicated(keep='last')]  # Remove duplicates from the dataset
            df2 = df2[~df2.index.isnull()]
            df = pd.concat([df, df2], axis=1)
