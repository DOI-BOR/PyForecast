    ...
    def load(self, dataset, date1, date2):
        ...
        if 'MB' in dataset.agency or 'GP' in dataset.agency:
            ...
            df = pd.read_csv(StringIO(response),
                             parse_dates=['DateTime'])  # Read the data into a dataframe
            df.set_index(pd.DatetimeIndex(pd.to_datetime(df['DateTime'])),
                         inplace=True)  # Set the index to the datetime column
            del df['DateTime']  # Delete the redundant datetime column
            df = df[
                ~df.index.duplicated(keep='first')]  # Remove duplicates from the dataset
            if 'monthly2daily' in dataset.opts:
                # print('INFO: Monthly data converted to daily')
                df[df.columns[0]] = pd.to_numeric(df[df.columns[0]],
                                                  errors='coerce')  # convert values to floats
                df = ConvertMonthlyToDaily(df)
            df = df[not df.index.isna()]
            df.columns = [
                'USBR | ' + dataset.external_id + ' | ' + dataset.param_code + ' | ' +
