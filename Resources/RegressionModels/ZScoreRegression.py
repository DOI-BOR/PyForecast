...
class Regressor:
    ...
    def train_model(self, x, y):
        ...
        self.masks = [~np.isnan(self.z_scr[:, i]) & ~np.isnan(y) for i in
                      range(self.z_scr.shape[1])]
        self.r2_scr = np.array([np.corrcoef(x=self.z_scr[self.masks[i]][:, i],
                                            y=y[self.masks[i]])[0, 1] ** 2 for i in
                                range(self.z_scr.shape[1])])
        numerator = np.nansum(self.r2_scr * self.z_scr, axis=1)
        divisors = np.nansum(np.where(~np.isnan(self.z_scr), self.r2_scr, np.nan),
                             axis=1)
        ...
        for ithTerm in range(0, len(self.r2_scr)):
            c = self.coef_[1] * self.r2_scr[ithTerm] / (
                        self.std[ithTerm] * np.nansum(self.r2_scr))
