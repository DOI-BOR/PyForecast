...
function createPopups() {
    ...
    window.layer_group.eachLayer(function (layer) {
        ...
        layer.on("click", function(e) {
            ...
            var units = e.layer.feature.properties.DatasetUnits;
            var pcode = e.layer.feature.properties.DatasetParameterCode;
            var dataloader = e.layer.feature.properties.DatasetDataloader;
            ...
            if (hiddenID.includes('|')) {
                popHTML = popHTML + `<p hidden id="parammeta">${id}~~${name}~~${agency}~~${dataloader}</p>`
                popHTML = popHTML + `</br><select id='param' onchange="updatePOR()">`;
                
                var params = param.split('|');
                var unitlist = units.split('|')
                var pcodes = pcode.split('|')
                params.forEach( function(parameter, idx){
                    popHTML = popHTML + `<option data-parameter="${parameter}" data-unit="${unitlist[idx]}" data-pcode="${pcodes[idx]}">${parameter}</option>`
                });
...
function buttonPress(grid_param=null) {
    ...
    if (data.nodeName.toUpperCase() == 'P') {
        ...
        if (selected.value == 'PRISM') {
            ...
            param = selected.dataset.parameter;
            unit = selected.dataset.unit;
            pcode = selected.dataset.pcode;
            id = document.getElementById('parammeta').innerHTML;
            id = id + `~~${param}~~${pcode}~~${unit}`
            console.log('ID:'+id);
        }
...
function PDSIPress() {
    ...
    var name = document.getElementById('meta-name').innerHTML;
    var select = document.getElementById('param')
    var param = select.options[select.selectedIndex].innerHTML;
