...
class ForecastViewer(QDialog):
    ...
    def load_sand(self):
        ...
        df = df.loc[convert_to_water_year(
            self.model.training_period_start):convert_to_water_year(
            self.model.training_period_end)].dropna()
        ...
        for i, d in enumerate(self.model.predictors):
            ...
            e.sand_ta.appendPlainText(
                f'{d.dataset().__export_form__()}: {d.data.loc[year]:0.2f} {d.unit.__str__()} \n\t\t-> [{pct} percent of normal using {per} years of data]')
    ...
    def disagg(self):
        ...
        df = pd.DataFrame()
        idx = pd.DatetimeIndex(pd.date_range(f'{syear}-{smnth:>02}-{sdy:>02}',
                                             f'{syear}-{emnth:>02}-{edy:>02}'))
        labels = []
        for exc, trace in self.traces[e.disagg_year_select.value()].items():
            for t in trace:
                ...
                labels.append(
                    f"{(100 * (1 - exc)):>02}%: Analog Year {trace[t]['analog_year']}")
        df.columns = labels
    ...
    def setForecast(self):
        ...
        for i in range(len(years)):
            ...
            self.data.append(
                [str(year), f'{actual:0.5g}', f'{predicted:0.5g}', f'{error:0.5g}',
                 forecast])

        for year in sorted(
                list(set(self.model.forecasts.forecasts.index.get_level_values(0)))):
...
class ModelPlots(pg.GraphicsLayoutWidget):
    ...
    def __init__(self):
        ...
        self.ep.sp.sigHovered.connect(
            lambda points, ev: self.hoverConnect(points, ev, 'ep'))
        self.tp.sp.sigHovered.connect(
            lambda points, ev: self.hoverConnect(points, ev, 'tp1'))
        self.tp.sp2.sigHovered.connect(
            lambda points, ev: self.hoverConnect(points, ev, 'tp2'))
