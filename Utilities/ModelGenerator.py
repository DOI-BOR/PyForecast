    ...
    def work(self):
        ...
        for predictor in self.config.predictor_pool.predictors:
            ...
            if not predictor.data.empty:
                predictor.data = predictor.data.loc[convert_to_water_year(
                    self.config.training_start_date):convert_to_water_year(
                    self.config.training_end_date)]

        self.config.predictand.resample()
        self.config.predictand.data = self.config.predictand.data.loc[
            convert_to_water_year(self.config.training_start_date):convert_to_water_year(
                self.config.training_end_date)]
        ...
        preproc_method = app.preprocessing_methods[
            'INV_' + self.config.predictand.preprocessing]
        ...
        for rr, regressor in enumerate(self.config.regressors):
            ...
            print(
                f"Beginning model search for regressor: {regressor.regression_model} / {regressor.scoring_metric}")
            ...
            feature_selector = app.feature_selection[feature_selector_name](self,
                                                                            configuration=self.config,
                                                                            num_predictors=len(
                                                                                self.config.predictor_pool) - num_forced - num_empty)
            ...
            while feature_selector.running or count == 0:
                ...
                self.updateProgSignal.emit(
                    int((past_prog + feature_selector.progress) / len(
                        self.config.regressors)))
                if predictors <= 0:
                    continue
                bool_index = feature_selector.convert_int_to_array(predictors,
                                                                   feature_selector.num_predictors)
                ...
                y_p, y_a = regression_algorithm.cross_val_predict(x_data,
                                                                  predictand_data)
                ...
                model = Model(
                    ...
                    predictors=list(
                        compress(self.config.predictor_pool.predictors, bool_index)),
                ...
                # if count%10 == 0:
                self.updateTextSignal.emit(
                    f'<b>{regressor.regression_model}</b><br>{genome} Scorer ({regressor.scoring_metric}): {score:+12.5f} ({len(feature_selector.completed):>12}/{feature_selector.num_possible})')
