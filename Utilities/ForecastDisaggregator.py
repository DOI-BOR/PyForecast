...
class ForecastDisaggregator(object):
    ...
    def Disaggregate(self, years, start_date, end_date):
        ...
        for year in years:
            ...
            exc_list = [round(i * 4) / 4 for i in
                        np.arange(start=1.25, stop=100, step=2.5)]
            for exc, val in self.forecasts.forecasts.loc[(year, slice(None))][
                'Value'].items():
        ...
        for exc in self.traces[year]:
            for n in self.traces[year][exc]:
                ...
                labels.append(label)
                data = self.traces[year][exc][n]['trace']
                data = data.set_axis(pd.DatetimeIndex(
                    pd.date_range(f'{year}-{start_date:%m-%d}',
                                  f'{year}-{end_date:%m-%d}')))
