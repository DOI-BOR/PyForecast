...
class Exporter:
    ...
    currentProgressChanged = Signal(int)
    ...
    def export(self) -> Path:
        ...
        fn = app.current_file.with_suffix('.xlsx')
        with pd.ExcelWriter(fn, engine='xlsxwriter') as writer:
            ...
            wrap_format = {'bold': True, 'bg_color': 'lime'}
            ...
            app.gui.status_bar.showMessage(
                f'Exporting to Excel (Datasets): 000/{self.num_datasets:>03}')
            app.processEvents()
            ...
            columns = [
                'Dataset GUID', 'Dataset ID', 'Name', 'Agency', 'Parameter',
                'Param Code', 'Raw Unit', 'Display Unit', 'Dataloader',
                'Data file'
            ]
            ...
            for i, dataset in enumerate(app.datasets.datasets):
                ...
                dataset_worksheet.write(i + 1, 6, dataset.raw_unit.id)

                col_widths[7] = max(col_widths[7], len(dataset.display_unit.id))
            ...
            app.gui.status_bar.showMessage(
                f'Exporting to Excel (Data): 000/{self.num_datasets:>03}')
            app.processEvents()
            ...
            for i, dataset in enumerate(app.datasets.datasets):
                app.gui.status_bar.showMessage(
                    f'Exporting to Excel (Data): {i + 1:>03}/{self.num_datasets:>03}'
                )
                app.processEvents()
            ...
            df.to_excel(writer, index=True, sheet_name='Data', header=False,
                        startrow=1, freeze_panes=(1, 1))
            ...
            # Saved Models
            app.gui.status_bar.showMessage(
                f'Exporting to Excel (Saved Models): 000/{self.num_models:>03}')
            app.processEvents()
            ...
            for year in app.saved_models.list_forecast_years():
                ...
                for i, model in enumerate(app.saved_models.saved_models):
                    if year in model.forecasts.forecasts.index.get_level_values(0):
                        ...
                        app.gui.status_bar.showMessage(
                            f'Exporting to Excel (Saved Models):'
                            f' {counter + 1:>03}/{self.num_models:>03}')
                        app.processEvents()
                        counter += 1

                        for j, col in enumerate(columns):
                            if col == 'Model Name':
                                saved_models_sheet.write(
                                    i + 1 - skipcount, j, model.name
                                )
                            elif col == 'Issue Date':
                                saved_models_sheet.write(
                                    i + 1 - skipcount, j,
                                    model.issue_date.strftime('%b %d')
                                )
                            elif col == 'Regressor':
                                saved_models_sheet.write(
                                    i + 1 - skipcount, j, model.regression_model
                                )
                            elif col == 'Cross Validation':
                                saved_models_sheet.write(
                                    i + 1 - skipcount, j, model.cross_validator
                                )
                            elif col == 'Forecast (50%)':
                                val = model.forecasts.get_10_50_90(year)[1]
                                saved_models_sheet.write(
                                    i + 1 - skipcount, j, f'{val:0.5g}'
                                )
                            elif col == "Predictors":
                                p_str = ', '.join(
                                    [p.dataset().external_id for p in model.predictors])
                                saved_models_sheet.write(i + 1 - skipcount, j, p_str)
                            elif col == 'Target':
                                saved_models_sheet.write(
                                    i + 1 - skipcount, j, model.predictand.__list_form__()
                                )
                            elif col.endswith('%'):
