...
class Exporter:
    ...
    def export(self):
        ...
        for i, dataset in enumerate(app.datasets.datasets):
            app.gui.status_bar.showMessage(
                f'Exporting to Excel (Datasets): {i + 1:>03}/{self.num_datasets:>03}')
            app.processEvents()
        ...
        for year in app.saved_models.list_forecast_years():
            ...
            for i, model in enumerate(app.saved_models.saved_models):
                if year in model.forecasts.forecasts.index.get_level_values(0):
                    ...
                    app.gui.status_bar.showMessage(
                        f'Exporting to Excel (Saved Models): {counter + 1:>03}/{self.num_models:>03}')
                    app.processEvents()
                    counter += 1
                    ...
                    for j, col in enumerate(columns):
                        if col == 'Model Name':
                            ...
                            saved_models_sheet.write(i + 1 - skipcount, j,
                            ...
                            saved_models_sheet.write(i + 1 - skipcount, j,
                                                     model.predictand.__list_form__())
                        elif col.endswith('%'):
