...
class DateTimeAxis(pg.AxisItem):
    ...
    def tickStrings(self, values, scale, spacing):
        ...
        for v in values:
            if v < 0:
                strings.append((datetime(1970, 1, 1) + timedelta(seconds=v)).strftime("%Y-%m"))
            else:
                if spacing > 7 * 365.24 * 86400:
                    strings.append(datetime.utcfromtimestamp(v).strftime('%Y-%m'))
                else:
                    strings.append(datetime.utcfromtimestamp(v).strftime('%Y-%m-%d'))
        return strings
...
class ResultsTabPlots(pg.GraphicsLayoutWidget):
    ...
    def appendForecast(self, fcastValue, fcastRange, fcastLabel=None):
        ...
        if fcastLabel is None:
            fcastLabel = f'Forecast ({"%.2f" % fcastValue})\n10%: {"%.2f" % fcastRange.loc[10]}\n90%: {"%.2f" % fcastRange.loc[90]}'
            #fcastLabel = 'Forecast (' + ("%.2f" % fcastValue) + ') '
        else:
            fcastLabel = str(fcastLabel) + f' Forecast ({"%.2f" % fcastValue})\n10%: {"%.2f" % fcastRange.loc[10]}\n90%: {"%.2f" % fcastRange.loc[90]}'
